...
Use the `AskUserQuestion` tool to gather optional context that materially changes the trace or summary. Goal is a more targeted output, not interrogation.

**Up front (high-level, before reading the full set):**
Ask 1-3 short questions when the answers aren't obvious from the documents:
- Audience: legal team / business stakeholder / partner-level review
- Output preference: narrative memo / clause-by-clause table / both
- Provisions that matter most to the user (if Mode 1 — full summary)
- Any known disputes or open issues tied to a specific provision

**Mid-flight (after reading the documents):**
After loading the agreements and amendments, surface concrete asks before guessing on:
- Conflicts between amendments (which controls if they disagree)
- Provisions that look orphaned (referenced but defined nowhere current)
- Whether to flag "stale but not deleted" language
- How aggressive to be on `[review]` flags vs silent normalization
...
```
/vallor:amendment-history acme-msa.pdf amendment-1.pdf amendment-2.pdf
```

```
/vallor:amendment-history --provision indemnity
```

```
/vallor:amendment-history
...
- It does not compare against the playbook in `~/.claude/plugins/config/vallor/CLAUDE.md` — that is the
