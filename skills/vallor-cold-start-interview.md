...
description: >
  ...
  `~/.claude/plugins/config/vallor/CLAUDE.md` is missing or still contains template
...
Runs the cold-start interview. First run writes `~/.claude/plugins/config/vallor/CLAUDE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.claude/plugins/config/vallor/CLAUDE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.claude/plugins/config/vallor/CLAUDE.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.claude/plugins/cache/vallor/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.claude/plugins/config/vallor/CLAUDE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.claude/plugins/config/vallor/CLAUDE.md` is written. What did I get wrong?"
   - Offer a test review: "Want to throw a contract at me?"
   - If a [CLM] is connected: offer to bulk-load the renewal register

## `--check-integrations`

Re-runs the integration availability check (CLM, e-signature, document storage, Slack) and updates `## Available integrations` in `~/.claude/plugins/config/vallor/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
## `--side sales` / `--side purchasing` / `--side procurement`

Re-runs only the relevant section of the interview, calibrated to the specified side, and writes the answers to the matching subsection in `~/.claude/plugins/config/vallor/CLAUDE.md`. Does NOT re-ask practice setting, role, integrations, or team details — those are side-agnostic.

- `--side sales` → builds `### Sales-side playbook` only.
- `--side purchasing` → builds `### Purchasing-side playbook` only.
- `--side procurement` → builds the entire `# Procurement & Strategic Sourcing Profile` section only (categories under management, sourcing approach, vendor classification, negotiation posture, spend analysis preferences, risk management, savings methodology, procurement house style).

Pick `--side procurement` when the user is a CPO / strategic sourcing manager who doesn't need the contract-review playbook configured — every procurement skill in the plugin only requires this section.

## Profile selection at first run

The first question the cold-start interview asks (after company-profile orientation) is which profile(s) to build:

> **Which profile(s) should I configure?**
> 1. **Contracts (sales-side)** — you sell your product/service. Mostly your paper.
> 2. **Contracts (purchasing-side)** — you buy from vendors. Mostly their paper.
> 3. **Contracts (both)** — you have both sales and purchasing flows.
> 4. **Procurement & strategic sourcing** — category strategy, RFQs, vendor management, negotiation prep, savings tracking.
> 5. **Both contracts + procurement** — the full lifecycle. Recommended for organizations that want one system from sourcing to signature to renewal.

Build the selected profile(s) in order. Each takes 10-15 minutes; the user can pause and resume.
...
```
/vallor:cold-start-interview
```

```
/vallor:cold-start-interview --redo
```

```
/vallor:cold-start-interview --check-integrations
```

```
/vallor:cold-start-interview --side purchasing
```

```
/vallor:cold-start-interview --side procurement
```

---

## Procurement intake (when `--side procurement` or selected at first run)

Run through these blocks. Each writes to the matching subsection of
`# Procurement & Strategic Sourcing Profile` in CLAUDE.md.

### Block P1 — Organization shape

1. Annual third-party spend? (Range is fine.)
2. Direct / indirect / services mix? (Rough %, %, %.)
3. Procurement function: centralized, decentralized, or center-led?
4. Procurement team size? Roles (category managers, sourcing managers, analysts)?
5. Reports to (CFO / COO / CPO / other)?
6. ERP / spend system in use (SAP Ariba, Coupa, Oracle, custom)?

### Block P2 — Categories under management

> "List your top 5-10 spend categories by annual spend. For each, tell me: annual spend, direct/indirect/services class, strategy owner. I'll fill in Kraljic quadrant by asking a couple of follow-up questions per category."

Build the categories table. For each, ask:
- Supply risk: high or low? (Many suppliers, easy switching = low. Few suppliers, switching cost = high.)
- Profit impact: high or low? (Big % of cost or product differentiation = high.)
- Map to Kraljic (Strategic / Leverage / Bottleneck / Non-critical).

### Block P3 — Sourcing approach

1. Default sourcing event style (RFI → RFP, direct RFQ, negotiation only)?
2. Bid floor — minimum suppliers in a competitive event? (Default 3.)
3. RFQ template format (Excel multi-sheet / Coupa / Ariba / Keelvar / SAP Sourcing)?
4. Default scoring weights (5 dimensions sum to 100%):
   - Capability & capacity
   - Technical / quality
   - Supply chain & logistics
   - Total cost of ownership
   - Partnership / strategic fit
5. TCO drivers always modeled? (Logistics, duties, quality, payment terms, currency — confirm or add.)

### Block P4 — Vendor / supplier classification

1. How do you classify suppliers today? (Strategic / preferred / transactional or other terminology?)
2. Consolidation principle for strategic categories: 1 supplier, 2, or 3+?

### Block P5 — Negotiation posture

1. Default approach (win-win / value-based / aggressive / depends on category)?
2. Concession discipline rule? (Always reciprocal? Tracked log?)
3. Walk-away authority by deal size (fill the four-row table):
   - < $250K → ?
   - $250K-$2M → ?
   - $2M-$10M → ?
   - > $10M → ?
4. Auction tools used (English / Dutch / Japanese / sealed-bid / none)?

### Block P6 — Spend analysis preferences

1. Base currency?
2. FX source (internal monthly / spot / proxy)?
3. Pareto threshold (default 0.80)?
4. Tail thresholds (short / classic / long — defaults: $25K-100K / $5K-25K / <$5K)?
5. Outlier method (IQR 1.5x or z-score 4σ)?

### Block P7 — Risk management

1. Which risk dimensions do you track? (Financial / geographic / quality / cyber / ESG — confirm or add.)
2. Where does the single-source register live today (CLM / Coupa / spreadsheet)?
3. Business continuity expectation for strategic suppliers?

### Block P8 — Savings methodology

1. Hard vs soft savings definition?
2. Baseline definition (last 12 months paid invoices? Prior PO? Market index?)?
3. Tracking system?
4. Reporting cadence (monthly / quarterly / annual)? To whom?

### Block P9 — House style

1. Where category strategies are stored?
2. Where RFQs and event records are stored?
3. Who receives stakeholder reports?
4. Board procurement readouts: when, format, level of detail?
...
Read `~/.claude/plugins/config/vallor/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.claude/plugins/cache/vallor/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
Look for `~/.claude/plugins/config/vallor/company-profile.md`.
...
>
> Quick or full? (Upgrade any time with `/vallor:cold-start-interview --full`.)
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/vallor:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.claude/plugins/config/vallor/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /vallor:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> - ✗ [Integration] — not found. [Feature] will fall back to [manual alternative]. [How to connect.] If you set this up later, re-run `/vallor:cold-start-interview --check-integrations`.
>
...
If the user picked **both**, run Part 2 once for sales-side now. Tell them: "We'll come back to purchasing-side with `/vallor:cold-start-interview --side purchasing` when we're done here." Write sales-side answers to `### Sales-side playbook`.
...
*Re-check: `/vallor:cold-start-interview --check-integrations`*
...
*[If not configured yet: leave the pointer "[Not configured — run /vallor:cold-start-interview --side sales to build it]" in place of the subsections below.]*
...
*[If not configured yet: leave the pointer "[Not configured — run /vallor:cold-start-interview --side purchasing to build it]" in place of the subsections below.]*
...
*To re-run the interview: `/vallor:cold-start-interview --redo`*
```
...
>
> - **Review a vendor MSA against your playbook** — e.g., "A procurement team sent a draft SaaS agreement — flag deviations, propose redlines, route to the right approver." Try: `/vallor:review`
> - **Triage an inbound NDA to GREEN / YELLOW / RED** — e.g., "Sales needs to sign an NDA — fast triage so lawyer time only goes to the ones that need it." Try: `/vallor:review`
> - **Track renewal deadlines** — e.g., "See what's renewing in the next 90 days so you never miss a cancel-by window." Try: `/vallor:renewal-tracker`
> - **Trace a clause across amendments** — e.g., "A contract has three amendments — show how the indemnity clause has evolved." Try: `/vallor:amendment-history`
> - **Escalate a deviation** — e.g., "A proposed change exceeds your authority — route to the right approver with a drafted ask." Try: `/vallor:escalation-flagger`
> - **Review pending playbook updates** — e.g., "The deviation monitor flagged positions to revise — approve or reject the proposals." Try: `/vallor:review-proposals`
>
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.claude/plugins/config/vallor/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
   > - Edit the file directly for a quick change (a new fallback, a revised threshold, a name swap)
   > - Run `/vallor:cold-start-interview --redo` for a full re-interview
   > - Run `/vallor:cold-start-interview --check-integrations` to re-check what's connected
   >
...
> - Run `/vallor:cold-start-interview --redo <section>` to re-interview one part, or edit the config file directly.
>
