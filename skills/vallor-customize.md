...
The user typed `/vallor:customize`. They want to change something
...
1. **Read the config.** Read
   ...
   (and `~/.claude/plugins/config/vallor/company-profile.md` one
   level up). If the plugin config does not exist or still contains
   `[PLACEHOLDER]` values, say:

   > You haven't run setup yet. Run `/vallor:cold-start-interview`
...
5. **For shared-profile changes** (company name, industry, jurisdictions,
   ...
   `~/.claude/plugins/config/vallor/company-profile.md` and note:
