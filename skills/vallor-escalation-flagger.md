...
description: >
  ...
  `~/.claude/plugins/config/vallor/CLAUDE.md`, and draft the ask. Use when the user
...
Names the approver for a contract issue per the `~/.claude/plugins/config/vallor/CLAUDE.md` escalation matrix and drafts the message so you're not writing "hey got a sec" at 5pm.

## Instructions

1. **Load `~/.claude/plugins/config/vallor/CLAUDE.md`** → Escalation section. If missing, say so — the practice profile needs editing.
...
Use the `AskUserQuestion` tool to confirm the framing before drafting the escalation. Escalations route to busy approvers; tone and ask shape whether you get a fast answer.

**Up front (before drafting):**
Ask 1-3 short questions when not obvious from the inputs:
- Urgency: same-day decision needed / this week / standard
- Confidence in the dollar/exposure figures (rough estimate vs confirmed)
- Channel preference: Slack short ping / Slack thread with detail / email memo
- Whether the approver has prior context on this counterparty (changes how much background to include)
- Recommended path: do you have a preference, or do you want the approver to choose

**Mid-flight (after matching to matrix):**
After identifying the approver, surface concrete asks before guessing on:
- Whether to copy a second approver (e.g., GC + CFO when dollar threshold straddles bands)
- How much business context the approver actually wants (a tight memo vs a fast yes/no question)
- Whether to attach the underlying review/redline or just the issue summary
...
Every contracts team has an escalation matrix, written or not. This skill reads the written one (in `~/.claude/plugins/config/vallor/CLAUDE.md`), matches a contract issue against it, names the approver, and drafts the ask so the lawyer isn't writing "hey do you have a sec" messages at 5pm.
...
Read `~/.claude/plugins/config/vallor/CLAUDE.md` → `## Escalation`. If it's missing or vague, say so — the cold-start interview should have captured this, and if it didn't, the practice profile needs editing.
...
Don't escalate things that are actually fine. If the term is within the fallbacks in `~/.claude/plugins/config/vallor/CLAUDE.md`, it doesn't need to go up.
...
Be specific. Not "escalate to legal leadership" — name the person or role from `~/.claude/plugins/config/vallor/CLAUDE.md`. If the matrix doesn't name anyone for this situation, say so: "The escalation matrix doesn't cover [situation]. Suggest asking [GC name] who owns this."
...
```markdown
**Escalating to:** [name]
**Via:** [Slack #channel / email / meeting — per `~/.claude/plugins/config/vallor/CLAUDE.md`]
...
**What our playbook says:** [quote from `~/.claude/plugins/config/vallor/CLAUDE.md`]
...
The calibration for what warrants escalation lives in `~/.claude/plugins/config/vallor/CLAUDE.md`, not in this skill. Check the playbook's stated position, its fallbacks, and its "automatic escalation regardless of dollar value" list:
...
If a term comes up that the playbook doesn't address, don't guess the threshold — ask the reviewing attorney whether this class of issue should escalate, and offer to record the answer in `~/.claude/plugins/config/vallor/CLAUDE.md` so future reviews are consistent.
