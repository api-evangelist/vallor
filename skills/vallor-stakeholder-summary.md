...
Read `~/.claude/plugins/config/vallor/CLAUDE.md` → `## House style` → who reads stakeholder summaries, how long should they be. If not specified, default to: procurement or a department head, two paragraphs max, no legal terms of art.
...
Use the `AskUserQuestion` tool to gather optional context the review memo doesn't reveal. A stakeholder summary lives or dies on calibration to the reader.

**Up front (before drafting the summary):**
Ask 1-3 short questions when not obvious from the review memo:
- Audience role: procurement / department head / finance / security/IT / executive sponsor / mixed
- Decision being asked: sign / push back / wait / informational only
- Length: tight one-pager / two paragraphs + checklist / full half-page
- Reader's technical depth: deep (knows legal terms) / standard / introductory (no jargon)
- Sensitivities: anything the user wants softened or amplified for THIS reader (e.g., budget exposure, renewal lock-in, data location)

**Mid-flight (after drafting the summary):**
After the first pass, surface concrete asks before guessing on:
- Whether to include or omit dollar exposure (some readers shouldn't see it)
- Whether to name escalation points by role (procurement lead) vs by name (Sarah)
- Whether to attach the underlying review memo or just link to it
...
**To the stakeholder:** Via whatever channel `~/.claude/plugins/config/vallor/CLAUDE.md` says. If Slack, keep it under 150 words. If email, the format above is fine as-is.
