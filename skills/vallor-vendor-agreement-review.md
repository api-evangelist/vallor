...
description: >
  ...
  `~/.claude/plugins/config/vallor/CLAUDE.md`. Flags deviations, assesses risk, generates
  specific redline language, and routes to the right approver. Loaded by
  /vallor:review when a vendor MSA, services agreement, or similar is detected.
...
Read a vendor agreement against the playbook this team actually uses (in `~/.claude/plugins/config/vallor/CLAUDE.md`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `~/.claude/plugins/config/vallor/CLAUDE.md`.** If it's missing or still has placeholders, surface this bounce:
...
>
> **Two choices:**
> - Run `/vallor:cold-start-interview` (2 minutes) to configure your profile, then I'll review tailored to YOUR playbook.
...
> "That was a generic run against default assumptions. Run `/vallor:cold-start-interview` to get output calibrated to YOUR practice — your playbook, your jurisdiction, your risk appetite. 2 minutes."
...
The playbook in `~/.claude/plugins/config/vallor/CLAUDE.md` is the source of truth. It tells you:
...
Check the "one thing" from `~/.claude/plugins/config/vallor/CLAUDE.md` first. If present:
...
For each playbook category in `~/.claude/plugins/config/vallor/CLAUDE.md`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `~/.claude/plugins/config/vallor/CLAUDE.md`]
...
**If they won't move:** [the fallback from `~/.claude/plugins/config/vallor/CLAUDE.md`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `~/.claude/plugins/config/vallor/CLAUDE.md`, or a deal-breaker. |
...
Severity is always applied *against `~/.claude/plugins/config/vallor/CLAUDE.md`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `~/.claude/plugins/config/vallor/CLAUDE.md`.
...
Check the escalation matrix in `~/.claude/plugins/config/vallor/CLAUDE.md` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `~/.claude/plugins/config/vallor/CLAUDE.md`. If the Role is Non-lawyer:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `~/.claude/plugins/config/vallor/CLAUDE.md`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `~/.claude/plugins/config/vallor/CLAUDE.md` house-style section.
...
- [ ] `~/.claude/plugins/config/vallor/CLAUDE.md` was loaded and quoted — not generic market positions
